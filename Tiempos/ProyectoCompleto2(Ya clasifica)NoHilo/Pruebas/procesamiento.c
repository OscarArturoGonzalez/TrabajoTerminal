...
// double WeightIH[NumInput+1][NumHidden+1]={{0,0.881821,2.751831,-0.849407,-3.233841,0.687500},
// 										  {0,-7.514493,-0.581251,6.691439,-3.658869,9.547394},
// 										  {0,-1.180301,0.518152,-4.345169,1.204935,-6.564086},
// 										  {0,0.054281,-2.467651,3.671744,-3.338704,-8.900757},
// 										  {0,2.442347,2.009274,9.436572,-4.752260,2.158721},
// 										  {0,0.830910,-1.111674,1.911683,-0.644509,0.652112},
// 										  {0,-1.930884,2.302066,-1.344696,1.868794,0.982207},
// 										  {0,2.449091,-1.102881,-3.003041,2.208487,-4.205731}};

// double WeightHO[NumHidden+1][NumOutput+1]={{0,-3.730119},             
// 										   {0,9.123057},
// 										   {0,-5.689060},	        
// 										   {0,-10.266590},
// 										   {0,-8.068258},
...
int main(void)
{
    ...
    int i, k;//nhs arreglo de identificadores para hilos, para cada hilo (correctamente)
	// pthread_t tids[NUM_HILOS]; //arreglo de identificadores 

	archivo = fopen("momentosHu.txt", "w+");
	archivo2 = fopen("clasificacion.txt", "w+");

	char carpeta[]="Imagenes1/";
	sprintf(str2,"%sbase.bmp",carpeta);

	imagenRGB = abrirBMP(str2, &info);	
 ...
 //    }
	...
	sprintf(str,"%sprueba%d.bmp",carpeta, cont);
...
long double normalizaPhi(long double phi, int moment){
	long double norm1, E;
	//long double DE[7]={0.1302,0.7378,1.1975,1.6583,20.3833,14.0535,23.1117}; **Entrenamiento2
...
void clasifica(void){
	...
	for( k = 1 ; k <= NumOutput ; k++ ) {         	/* k es el  indice de las salidas */
		...
		Output[1] = 1.0/(1.0 + exp(-SumO[k]));  	//Activacion con sigmoide
		printf("Output= %lf \n", Output[k]); //%lf
	}
...
void filtroGaussiano(void)
{
 ...
 //    int finBloque=iniBloque+tamBloque; 
	...
	for(y=0;y<info.height-DIMASK;y++) //llega hasta el tamaño-(DIMASK)::y=inibloq-numerodehilo*tamanio de mascara::
...
register int y,x,ym,xm;
    ...
    // int finBloque=iniBloque+tamBloque;   
        ...
        for(y=0; y<info.height-bloque;y++){               //recorrer imagen
            for(x=0; x<info.width-bloque; x++){                             
                media=0.0;
                desv=0.0;
                umbral=0;
