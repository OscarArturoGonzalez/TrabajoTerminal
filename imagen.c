...
unsigned char *abrirBMP(char *filename, bmpInfoHeader *bInfoHeader) //lee y regresa la imagen en un apuntador 
{
    ...
    imgsize=(bInfoHeader->width)*(bInfoHeader->height)*3;
    ...
    //printf("imgsize: %d \n", bInfoHeader->imgsize); //El imgsize es 0 en las raspistill se requiere
    /* Leemos los datos de imagen, tantos bytes como imgsize */
    fread(imgdata, imgsize,1, f);
...
void guardarBMP( char *filename, bmpInfoHeader *info, unsigned char *imgdata )
{
    ...
    header.size = imgsize + sizeof(bmpFileHeader) + sizeof(bmpInfoHeader);
    /* header.resv1=0; */
    /* header.resv2=1; */
