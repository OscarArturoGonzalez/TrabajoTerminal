...
int main(void)
{
	...
	char carpeta[]="Imagenes1/";  //<<---------Aqui se cambia la carpeta
	...
	imagenGray=RGBToGray(imagenRGB, info.width, info.height);
	//GrayToRGB(imagenRGB, imagenGray, info.width, info.height);
	//sprintf(str2,"Resultados/gris%d.bmp",cont);
	...
	//GrayToRGB(imagenRGB, imagenAux, info.width, info.height);
	//sprintf(str2,"Resultados/filtroB%d.bmp",cont);
	//guardarBMP(str2, &info, imagenRGB);
...
long double normalizaPhi(long double phi){
	//return (-1*((phi > 0) ? 1 : ((phi < 0) ? -1 : 0))) * (log10(fabs(phi)))/35;
	return (-1*((phi > 0) ? 1 : ((phi < 0) ? -1 : 0))) * (log10(((phi > 0) ? 1*phi : ((phi < 0) ? -1*phi : 0)))); 
}
